...
if MODE == 'KAGGLE-VAL':
    ...
    ROI_YOLOX_ENGINE_PATH = '../roi_det/YOLOX/YOLOX_outputs/yolox_nano_bre_416/model_trt.pth'
    CSV_PATH = '/home/dangnh36/projects/.comp/rsna/datasets/external/vindr/all_labels.csv'
    DCM_ROOT_DIR = '/home/dangnh36/projects/.comp/rsna/datasets/external/vindr/images'
    SAVE_IMG_ROOT_DIR = '/raid/.comp/external/vindr/test_uint8_hasvoi_png@yolox_nano_bre_416_datav2'
    N_CHUNKS = 1
    N_CPUS = 4
    RM_DONE_CHUNK = False
else:
...
def _single_decode_crop_save_sdl(roi_extractor,
    ...
    # dcm.copyFrameData(index, img)
    # # img = apply_voi(img, ds, index = 0).astype(np.int16)
    ...
    try:
        ...
        if xyxy is not None:
            ...
            print(f'Can not detect: {save_path}')
            crop = img_torch
    except:
        print('ROI extract exception!')
        crop = img_torch
    ...
    if 1:
        ...
        for name in remain_img_names:
            ...
            remain_dcm_paths.append(
                os.path.join(DCM_ROOT_DIR, patient_id, f'{image_id}.{DCM_EXTENSION}'))
