...
if MODE == 'KAGGLE-VAL':
    ...
    ROI_YOLOX_ENGINE_PATH = '../roi_det/YOLOX/YOLOX_outputs/yolox_nano_bre_416/model_trt.pth'
    CSV_PATH = '/home/dangnh36/projects/.comp/rsna/datasets/external/cdd_cesm/label_v2.csv'
    DCM_ROOT_DIR = '/home/dangnh36/projects/.comp/rsna/datasets/external/cdd_cesm/low_energy'
    SAVE_IMG_ROOT_DIR = '/raid/.comp/external/cdd_cesm/uint8_voilut_png@yolox_nano_bre_416_datav2'
    N_CHUNKS = 1
    N_CPUS = 4  
    RM_DONE_CHUNK = False
else:
...
def percentile_min_max_scale(img):
    maxv = np.percentile(img, 99) - 1
    minv = img.min()
    assert maxv >= minv
    if maxv > minv:
...
for chunk_idx, chunk_patients in enumerate(chunk_patients):
    ...
    for i in range(len(df)):
        patient_id = df.at[i, 'patient_id']
        image_id = df.at[i, 'image_id']
        dcm_image_id = image_id.replace('_CC', '_DM_CC').replace('_MLO', '_DM_MLO')
        dcm_path = os.path.join(DCM_ROOT_DIR,
                                f'{dcm_image_id}.{DCM_EXTENSION}')
    ...
    if 1:
        ...
        for name in remain_img_names:
            ...
            remain_dcm_paths.append(
                os.path.join(DCM_ROOT_DIR, f'{dcm_image_id}.{DCM_EXTENSION}'))
