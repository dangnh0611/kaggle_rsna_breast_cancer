...
class BalanceSamplerV2(Sampler):
    def __init__(self, dataset, batch_size = 8, start_ratio = 1/4, end_ratio = 1/8, num_epochs = 4):
        self.dataset = dataset
        ...
        self.pos_index = np.where(labels>0)[0]
        self.neg_index = np.where(labels==0)[0]
        print(f'Num pos: {len(self.pos_index)}, Num neg: {len(self.neg_index)}')

        # percentage of pos samples per epoch
        self.ratios = np.linspace(start_ratio, end_ratio, num_epochs)
        self.cur_ratio = None


    def set_epoch(self, ep):
        if ep >= self.num_epochs:
            assert self.ratio == self.ratios[-1]
        else:
            self.ratio = self.ratios[ep]
        print('Set sampler ratio to:', self.ratio)
...
class BalanceSamplerV3(Sampler):
    def __init__(self, dataset, batch_size, num_sched_epochs, num_epochs, start_ratio = 1/4, end_ratio = 1/8, one_pos_mode = True, seed = 42):
        ...
        # labels = np.concatenate([np.ones((1000,)), np.zeros((43000,)), ], axis = 0)
