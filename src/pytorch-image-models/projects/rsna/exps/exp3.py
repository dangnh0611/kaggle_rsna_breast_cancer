...
class Exp(_BaseExp):
    ...
    def compute_metrics(self,
        ...
        all_metrics.update({f'single_best_{k}': v for k, v in metrics['best_metric'].items()})
        all_metrics['single_pfbeta'] = metrics['pfbeta']
        all_metrics['single_auc'] = metrics['auc']

        # groupby mean
        mean_df = df[['patient_id', 'laterality', 'preds', 'targets']].groupby(['patient_id', 'laterality']).mean()
        mean_preds = mean_df['preds'].to_numpy()
        mean_gts = mean_df['targets'].to_numpy()
        # mean_sample_weights = mean_df['sample_weights']
        mean_metrics = self._compute_metrics(mean_gts, mean_preds, None, thres_range, sort_by)
        all_metrics['gbmean_best_thres'] = mean_metrics['best_thres']
        all_metrics.update({f'gbmean_best_{k}': v for k, v in mean_metrics['best_metric'].items()})
        all_metrics['gbmean_pfbeta'] = mean_metrics['pfbeta']
        all_metrics['gbmean_auc'] = mean_metrics['auc']

        # groupby max
        max_df = df[['patient_id', 'laterality', 'preds', 'targets']].groupby(['patient_id', 'laterality']).max()
        max_preds = max_df['preds'].to_numpy()
        max_gts = max_df['targets'].to_numpy()
        # max_sample_weights = max_df['sample_weights']
        max_metrics = self._compute_metrics(max_gts, max_preds, None, thres_range, sort_by)
        all_metrics['gmax_best_thres'] = max_metrics['best_thres']
        all_metrics.update({f'gbmax_best_{k}': v for k, v in max_metrics['best_metric'].items()})
