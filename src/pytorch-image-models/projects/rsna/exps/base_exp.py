...
class Exp(_BaseExp):
    ...
    def compute_metrics(self,
        ...
        ori_df = df[['site_id', 'patient_id', 'laterality', 'cancer', 'preds', 'targets']]
        all_metrics = {}
        ...
        reducer_gbmean_site1 = lambda df: df[df.site_id == 1].reset_index(drop = True).groupby(['patient_id', 'laterality']).mean()
        reducer_gbmean_site2 = lambda df: df[df.site_id == 2].reset_index(drop = True).groupby(['patient_id', 'laterality']).mean()
        ...
        for reducer_name, reducer in reducers.items():
            ...
            all_metrics.update({f'{reducer_name}_best_{k}': v for k, v in _metrics['best_metric'].items()})
    ...
    def _compute_metrics(self,
        ...
        # PR-AUC
        precisions, recalls, _thresholds = sklearn.metrics.precision_recall_curve(gts, preds)
