...
from timm.loss import (BinaryCrossEntropy, BinaryCrossEntropyPosSmoothOnly,
...
class Exp(_BaseExp):
    ...
    def __init__(self, args):
        ...
        print('\n------\nEXP METADATA:\n', self.meta)
        self.output_dir = os.path.join(SETTINGS.MODEL_CHECKPOINT_DIR,
                                       'timm_classification')
    ...
    def build_train_dataset(self):
        ...
        for dataset_name in EXTERNAL_DATASETS:
            dataset_info = {
                'csv_path':
                os.path.join(SETTINGS.PROCESSED_DATA_DIR, 'classification',
                             dataset_name, 'cleaned_label.csv'),
                'img_dir':
                os.path.join(SETTINGS.PROCESSED_DATA_DIR, 'classification',
                             dataset_name, 'cleaned_images')
            }
            # small change for fully reproducible
            if dataset_name == 'bmcd':
                dataset_info['csv_path'] = os.path.join(SETTINGS.ASSETS_DIR, 'data', 'bmcd_cleaned_label.csv')
            elif dataset_name == 'cmmd':
                dataset_info['csv_path'] = os.path.join(SETTINGS.ASSETS_DIR, 'data', 'cmmd_cleaned_label.csv')
                
            train_datasets_info.append((dataset_name, dataset_info))

        train_dataset = datasets.v3.RSNADataset(
            train_datasets_info,
            augment_fn,
            transform_fn,
