...
# ./distributed_train_exp.sh 4 -f projects/rsna/exps/exp1.py --model tf_efficientnet_b4.ns_jft_in1k --pretrained --sync-bn --num-classes 2 --batch-size 12 --validation-batch-size 16 --opt sgd --lr 0.0001 --min-lr 0.000001 --sched cosine --warmup-lr 0.000001 --epochs 100 --warmup-epoch 15 --cooldown-epochs 5 --no-aug --crop-pct 1.0 --bce-loss --eval-metric loss --log-interval 20 --workers 8 --smoothing 0.1 --img-size 1024 --amp --amp-impl native --eval-metric best_pfbeta --save-images --checkpoint-hist 100
#  ./distributed_train_exp.sh 4 -f projects/rsna/exps/exp1.py --model tf_efficientnet_b2.ns_jft_in1k --pretrained --sync-bn --num-classes 2 --batch-size 6 --validation-batch-size 8 --opt sgd --lr 0.0001 --min-lr 0.000001 --sched cosine --warmup-lr 0.000001 --epochs 50 --warmup-epoch 5 --cooldown-epochs 1 --no-aug --crop-pct 1.0 --bce-loss --eval-metric loss --log-interval 20 --workers 8 --smoothing 0.1 --img-size 2048 --amp --amp-impl native --eval-metric best_pfbeta --save-images --checkpoint-hist 100
...
group.add_argument('--checkpoint-hist', type=int, default=99999, metavar='N',
...
group.add_argument('--dense-ckpt-epochs', default=[9999, -1], nargs=2, type=int,
                   metavar='N N',
                   help='Validate and save ckpts more frequently (validate >1 times per epoch)')
group.add_argument('--dense-ckpt-bins', type=int, default=4, metavar='N',
                   help='Validate and save ckpts more frequently (validate >1 times per epoch)')                   
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in tqdm(enumerate(loader)):
        ...
        ##########################################################
        ##########################################################
        # perform validate and save model checkpoints
        # START
...
def validate(
    ...
    # best_thres = metric_results['gbmean_best_thres']
